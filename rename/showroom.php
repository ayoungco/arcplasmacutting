...
if( $action == 'album'){
	...
	// This is where the blank thumbnail issue is occurring. Sometimes there will not be a 6th image entry
   ...
   //once the image was clicked
    ...
    foreach($init as $row) {
		...
		$thumbnail = NULL;
		$k = 5;
		while (!isset($thumbnail)) {			
			$thumbnail = $row[images][$k][source];
			$k--;
			// remove this
			// echo $thumbnail;
		}
		// -->
        echo "<a class=\"gallery_photos\" style='padding:10px; margin:3px; width: 190px; height: 190px; float: left; background-image:url(".$thumbnail."); background-repeat:no-repeat; background-position:center center;' href=\"" . $row['source'] . "\" title=\"" . $row['name'] . "\" rel=\"lightbox\">";
        echo "</a>";
    }
